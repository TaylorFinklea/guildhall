...
- [ ] **`gauntlet-90e` (P2)** — replay never captures token cost (`from_replay` hardcodes `0.0`); lanes reporting `cost:0` (opencode-go, ollama-cloud) mean *unreported*, not free. Prereq for any controlled same-task A/B efficiency study.
