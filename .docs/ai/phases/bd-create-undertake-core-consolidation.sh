...
SPEC="$ROOT/guildhall/.docs/ai/phases/undertake-core-consolidation-spec.md"
...
HISTORICAL_CLOSED_IDS=(
  undertake/conductor-run-contract
  undertake/conductor-bursar-roster
  undertake/conductor-arena-loop
  undertake/conductor-eval-fold
  musterroll/bursar-roster-contract
  musterroll/bursar-roster-migrate
  musterroll/bursar-roster-snapshot
  afterfact/hindsight-conductor-runs
  afterfact/hindsight-store
  afterfact/hindsight-ingest
  afterfact/hindsight-event-v2
  cautionlight/warden-findings
  cautionlight/warden-readonly-cutover
)
...
preserve_closed_bead undertake conductor-run-contract
preserve_closed_bead undertake conductor-bursar-roster
preserve_closed_bead undertake conductor-arena-loop
preserve_closed_bead undertake conductor-eval-fold
preserve_closed_bead musterroll bursar-roster-contract
preserve_closed_bead musterroll bursar-roster-migrate
preserve_closed_bead musterroll bursar-roster-snapshot
preserve_closed_bead afterfact hindsight-conductor-runs
preserve_closed_bead afterfact hindsight-store
preserve_closed_bead afterfact hindsight-ingest
preserve_closed_bead afterfact hindsight-event-v2
preserve_closed_bead cautionlight warden-findings
preserve_closed_bead cautionlight warden-readonly-cutover

# ---------------------------------------------------------------------------
# Undertake: one explicit verified job-loop kernel.
# ---------------------------------------------------------------------------


create_bead undertake undertake-run-v2 1 300 lead XL \
  'cargo test run && cargo test quarantine && cargo test musterroll && cargo clippy --all-targets -- -D warnings' \
  'Cut Undertake to strict v2 roster and run contracts' \
  'Read the consolidation spec Stable process contracts and role-aware plan-routing sections first. Files: src/musterroll.rs, src/run.rs, src/quarantine.rs, src/dispatch_cycle.rs, src/route.rs, and focused schema fixtures. Consume only musterroll/roster@2, validate policy_sha256, copy and pin the exact snapshot bytes per prepared run, and write deny-unknown-fields undertake/run@2 plus undertake/event@2 artifacts under runs-v2. Make job/details, targets, constrained stage routes, and plan progress structural. Quiesce and drain v1 pending, implementing, or reclaimable state before activating the v2 scanner; never scan legacy runs or parse v1/Arena state.' \
  ...
  'cross-repo-gate: musterroll-roster-v2-snapshot (publishes musterroll/roster-config@2 and musterroll/roster@2); closed conductor-run-contract and conductor-bursar-roster remain historical evidence'

create_bead undertake undertake-role-routing 1 300 lead XL \
  ...
  'Read the consolidation spec Role-aware plan routing section first. Files: src/config.rs, src/musterroll.rs, src/route.rs, src/run.rs, undertake.toml, and focused scheduler/lock modules. Parse strict generic role/profile bindings with nonzero weights and a digest over canonical policy plus the pinned Musterroll policy digest. Bind openai-codex--omp--gpt-5.6-sol--xhigh at 60, anthropic--omp--claude-opus-4-8--max at 20, and opencode-go--omp--kimi-k3--max at 20 for the initial plan pool. Implement deterministic smooth weighted round-robin in independent role/stage lanes under fs2 guards. Apply hard eligibility before scoring; persist checked scores, sequence, and irreversible PendingApproval/Committed/Canceled reservations linked to preallocated RunIds. Pin complete planner/peer/second-opinion candidate sets and relational constraints; do not read OMP personal role fallbacks or move policy into Musterroll.' \
  ...
  'cross-repo-gate: musterroll-roster-v2-snapshot with strict role capabilities and the three approved OMP plan profiles; depends-on: undertake-run-v2'

create_bead undertake undertake-job-registry 1 120 lead M \
  'cargo test job' \
  'Add the closed work review consult and plan job registry' \
  'Read the consolidation spec Undertake loop and job model section first. Files: src/cli.rs, src/config.rs, focused job modules, and templates. Add the closed JobKind set work, review, consult, and plan against strict v2 run details. Review retains the provider-diverse N-reviewer plus independent-judge contract. Plan is a distinct bounded job, not a hidden work stage or second loop engine. Config binds jobs to Musterroll profile IDs, mutation posture, limits, verifier, approval, and the generic role-policy seam without adding a workflow language, plugin loader, compatibility parser, or fleet-scan behavior.' \
  'Exactly work, review, consult, and plan parse and validate against a pinned Musterroll v2 snapshot; Arena and unknown jobs fail as usage/config errors. Job-tagged details cannot cross variants, read-only jobs cannot request write-capable execution, plan cannot mutate its target, and explain output shows pinned selection and fallback/constraint reasons. Existing review behavior remains N-plus-one and legacy CLI tests stay green.' \
  $'depends-on: undertake-run-v2\nThe validated review binding remains the operational source for the provider-diverse N-reviewer panel plus independent Lead judge. Generic plan role/profile weights and stage constraints belong to Undertake role policy; Musterroll v2 supplies only exact identities, capabilities, and eligibility.'

create_bead undertake undertake-loop-kernel 1 180 lead L \
  'cargo test loop' \
  'Build the native fresh-context resumable explicit-target loop kernel' \
  'Read the consolidation spec Undertake loop and job model section and the current Ralph driver before editing. Files: src/run.rs, src/dispatch.rs, src/verify.rs, src/plan.rs, src/cli.rs, and focused new loop code. Build on strict v2 run/event state and implement one explicit repo plus Bead or artifact target, one fresh harness context per bounded iteration, durable state before and after every subprocess, resume and reclaim, identity-checked commit success, exclusive repo lease, bounded failure continuation, and verifier-gated terminal completion. Reuse existing Exec and verifier seams; do not shell out to Ralph as the steady-state engine.' \
  ...
  'existing correctness gates: conductor-1i9 conductor-vnu conductor-9uk conductor-cwl conductor-wxx; depends-on: undertake-run-v2 undertake-job-registry'

create_bead undertake undertake-adversarial-job 1 120 senior M \
  'cargo test adversarial' \
  'Preserve adversarial review as the Undertake review job' \
  ...
  $'existing gates: conductor-vly conductor-j84 conductor-zg9 conductor-5tg conductor-z8z; depends-on: undertake-loop-kernel\nDiversity acceptance clarification 2026-07-17: for the same reviewed target, the approved panel must include Fable 5 when eligible plus at least one positively eligible non-Anthropic execution profile; the Lead judge must be independent of the implementation profile and preserve anonymous synthesis/minority findings. Terra/Luna implementation and Ollama Cloud GLM-5.2/MiniMax M3 review calls remain exact profile-distinct evidence in Afterfact. If the panel and judge cannot satisfy positive eligibility/provider-diversity, fail closed before dispatch.\nBinding ownership: conductor-pzo owns the exact Fable and provider-diverse profile configuration after this job exists; this bead owns behavioral parity and fail-closed review mechanics, not hard-coded model names.'

create_bead undertake undertake-consult-job 2 120 senior M \
  'cargo test consult' \
  'Fold the Envoy read-only evidence contract into the consult job' \
  'Read the corrected Envoy corpus and consolidation consult-job rules first. Files: src/cli.rs, job code, templates, schema-validation code, and tests/fixtures. Import the consult prompt and evidence-or-gaps envelope as one read-only Undertake job. Treat question, schema, target repo contents, and embedded directives as untrusted data. The job may write only its own Undertake run artifacts.' \
...
create_bead undertake undertake-plan-job 1 360 lead XL \
  ...
  'cross-repo-gate: musterroll-roster-v2-snapshot through undertake-role-routing; depends-on: undertake-loop-kernel undertake-run-v2 undertake-role-routing'

create_bead undertake undertake-plan-review-eval-fold 2 180 lead L \
...
create_bead musterroll musterroll-roster-v2-migrate 1 120 senior M \
  'cargo test roster_migration' \
  'Migrate the immutable Musterroll v1 identity set into the v2 roster' \
...
create_bead afterfact afterfact-ingest 1 180 senior L \
  'cargo test ingest' \
  'Implement incremental idempotent Afterfact ingestion' \
  ...
  'existing gates: hindsight-d96 hindsight-byi hindsight-6h8 hindsight-976; depends-on: afterfact-store'

create_bead afterfact afterfact-event-v2 1 120 lead M \
  'cargo test event_stream' \
  'Publish afterfact/event@2 with source artifact identity' \
  'Read src/event.rs, src/cli.rs, the Guildhall stdout-layer ADR, and the consolidation Afterfact event stream section first. Emit one versioned envelope per line with stable event ID, normalized redacted event, and canonical raw source artifact path plus SHA-256. Surface coverage gaps on the events path. Keep a bounded explicit v1 compatibility flag until consumers migrate; unknown filters and schemas fail visibly; preserve SIGPIPE-safe writes.' \
  'Default output validates as afterfact/event@2; raw_ref remains exact path and line; artifact identifies the raw source rather than SQLite; unsafe tool input stays opt-in with warning; gaps are not discarded; unknown schema consumers can reject deterministically; pipe to head exits cleanly; v1 compatibility is separately tested and marked temporary.' \
  'existing gates: hindsight-3kn hindsight-vxd; depends-on: afterfact-ingest'

create_bead afterfact afterfact-undertake-runs-v2 1 120 senior M \
  'cargo test undertake_source' \
  'Ingest strict Undertake v2 run manifests and events into Afterfact' \
  'Read strict undertake/run@2 and undertake/event@2 from the consolidation spec plus the existing Afterfact source-module pattern. Add a separate active-v2 Undertake source parser that validates schema, sequence, copied roster snapshot and policy digest, artifact hashes, exact profile/execution/provider identity, job, role, typed stage, target, attempts, verifier/reviewer results, usage, and terminal outcome. Store run and attempt rows without importing raw stdout or prompts; reject v1/Arena rows in the active v2 source. Preserve hindsight-conductor-runs as immutable closed v1 evidence.' \
  ...
  'cross-repo-gate: undertake-run-v2; depends-on: afterfact-store afterfact-ingest; closed hindsight-conductor-runs remains immutable v1 evidence'

create_bead afterfact afterfact-observations 1 180 senior L \
  'cargo test observations && cargo test legacy_scorecard' \
  'Add canonical observations and import legacy scorecard evidence' \
  'Read the consolidation Scorecards section, current model-scorecard and model-bench formats, and Musterroll append-only observation precedent first. Add afterfact/observation@1, a bounded single-write append CLI, corrections by reference rather than mutation, and a one-time idempotent importer for model-bench JSONL plus Experience Log entries. Store canonical records in ~/.local/state/afterfact/observations.jsonl and derived rows in SQLite.' \
  ...
  'depends-on: afterfact-store afterfact-ingest'

create_bead afterfact afterfact-scorecards 1 180 lead L \
  'cargo test scorecard' \
  'Derive model harness profile and job scorecards in Afterfact' \
  ...
  $'depends-on: afterfact-undertake-runs-v2 afterfact-observations\nUser comparison requirement 2026-07-17: retain exact execution profiles so Fable 5 review, Terra/Luna implementation, Ollama Cloud GLM-5.2, Ollama Cloud MiniMax M3, and same-base models on other providers remain separately comparable by job and task stratum. Every launched Undertake call, including repairs and judges, counts as an attempt; do not collapse a multi-call Bead into one opaque row.'

create_bead afterfact afterfact-scorecard-publish 2 120 senior M \
  'cargo test scorecard_publish' \
  'Publish Afterfact scorecards and evidence-pinned roster recommendations' \
  'Read src/deck.rs, harness-deck report@1, the old generator outputs, and the consolidation feedback-loop rules first. Add afterfact scorecard publish for evergreen model and harness reports and afterfact roster recommend for a versioned JSON proposal. The recommendation pins query window, evidence hash, affected Musterroll profile IDs, before/after values, metrics, sample warnings, and coverage gaps. It never edits Musterroll config.' \
  'Generated model and harness reports validate with the repo fixture validator; repeated generation is deterministic; real-only cost rules remain; recommendations below evidence floors are withheld with reasons; no Musterroll or Undertake file changes; publisher failure does not corrupt the store.' \
  $'depends-on: afterfact-scorecards\nCompatibility-removal gate: this published Afterfact report replaces the legacy chezmoi scorecard digest only after parity is pinned. Its evidence is required by conductor-7rs before Undertake stops model-bench dual writes; Musterroll recommendations remain proposal-only.'

create_bead afterfact afterfact-attribution 1 180 lead L \
  'cargo test attribution' \
  'Fold corrected Provenance attribution and queries into Afterfact' \
  'Read the corrected Provenance migration corpus, src/correlate.rs, src/confidence.rs, src/query.rs, and the consolidation Provenance section first. Port correlation strategies, confidence, surviving-hunk attribution, coverage accounting, and unreviewed-junior query into the Afterfact store and CLI. Use stored event/raw artifact evidence; false attribution remains worse than no attribution. Import existing sidecars only as migration input.' \
  ...
  'cross-repo-gate: provenance-afterfact-corpus; existing gates: hindsight-pov hindsight-w5w; depends-on: afterfact-event-v2'

# ---------------------------------------------------------------------------
# Cautionlight: a stateless advisory filter.
# ---------------------------------------------------------------------------

create_bead cautionlight cautionlight-findings 1 120 lead M \
  'cargo test findings' \
  'Consume Afterfact events and emit read-only Cautionlight findings' \
  'Read core/src/classify.rs, core/src/policy.rs, core/src/audit.rs, and the consolidation Cautionlight findings contract first. Add a small cautionlight CLI workspace member with inspect --stdin. Validate afterfact/event@2, apply a bounded set of high-signal existing rules, and emit cautionlight/finding@1 JSONL with input event IDs, rule, severity, claim, evidence, and gaps. Keep the operation stateless and read-only.' \
  'Golden benign, suspicious, malformed, unknown-schema, and coverage-gap streams produce deterministic findings and exit codes; valid findings exit zero, incomplete coverage exits one, usage/schema failure exits two; stdout contains JSONL only; no repo, hook config, Afterfact store, or external state is written.' \
  'cross-repo-gate: afterfact-event-v2'

create_bead cautionlight cautionlight-readonly-cutover 2 60 senior S \
  'cargo test --workspace && cargo run -p cautionlight -- inspect --help' \
  'Make read-only batch advice the supported Cautionlight product' \
  'Read AGENTS.md, README and handoff docs, adapter docs, and the consolidation Cautionlight decision first. Document afterfact events piped to cautionlight inspect as the supported path. Mark the Claude PreToolUse adapter experimental and uninstalled, remove pending installation instructions from active roadmap/current state, and preserve its code only as historical compatibility until archive review. Do not add enforcement.' \
  ...
  'depends-on: cautionlight-findings'

# ---------------------------------------------------------------------------
# Corrected parity corpora from the retiring satellites.
# ---------------------------------------------------------------------------

create_bead provenance provenance-afterfact-corpus 1 120 lead M \
  'cargo test correlation_corpus' \
  'Freeze corrected Provenance attribution behavior for Afterfact migration' \
...
add_dep undertake undertake-run-v2 conductor-ldq
add_dep undertake undertake-run-v2 conductor-z90
add_dep undertake undertake-run-v2 conductor-ldz
add_dep undertake undertake-run-v2 conductor-0ma
add_dep undertake undertake-run-v2 conductor-1br
add_dep undertake undertake-role-routing undertake-run-v2
add_dep undertake undertake-job-registry undertake-run-v2
add_dep undertake undertake-loop-kernel undertake-run-v2
add_dep undertake undertake-loop-kernel undertake-job-registry
add_dep undertake undertake-loop-kernel conductor-1i9
add_dep undertake undertake-loop-kernel conductor-vnu
add_dep undertake undertake-loop-kernel conductor-9uk
add_dep undertake undertake-loop-kernel conductor-cwl
add_dep undertake undertake-loop-kernel conductor-wxx
add_dep undertake undertake-adversarial-job undertake-loop-kernel
add_dep undertake undertake-adversarial-job conductor-vly
add_dep undertake undertake-adversarial-job conductor-j84
add_dep undertake undertake-adversarial-job conductor-zg9
add_dep undertake undertake-adversarial-job conductor-5tg
add_dep undertake undertake-adversarial-job conductor-z8z
add_dep undertake undertake-consult-job undertake-loop-kernel
add_dep undertake undertake-plan-job undertake-loop-kernel
add_dep undertake undertake-plan-job undertake-run-v2
add_dep undertake undertake-plan-job undertake-role-routing
add_dep undertake undertake-plan-review-eval-fold undertake-plan-job
add_dep undertake undertake-plan-review-eval-fold undertake-adversarial-job

add_dep musterroll musterroll-roster-v2-migrate musterroll-roster-v2-contract
add_dep musterroll musterroll-roster-v2-snapshot musterroll-roster-v2-migrate
add_dep musterroll musterroll-roster-v2-snapshot bursar-trz

add_dep afterfact afterfact-ingest afterfact-store
add_dep afterfact afterfact-ingest hindsight-d96
add_dep afterfact afterfact-ingest hindsight-byi
add_dep afterfact afterfact-ingest hindsight-6h8
add_dep afterfact afterfact-ingest hindsight-976
add_dep afterfact afterfact-event-v2 afterfact-ingest
add_dep afterfact afterfact-event-v2 hindsight-3kn
add_dep afterfact afterfact-event-v2 hindsight-vxd
add_dep afterfact afterfact-undertake-runs-v2 afterfact-store
add_dep afterfact afterfact-undertake-runs-v2 afterfact-ingest
add_dep afterfact afterfact-observations afterfact-store
add_dep afterfact afterfact-observations afterfact-ingest
add_dep afterfact afterfact-scorecards afterfact-undertake-runs-v2
add_dep afterfact afterfact-scorecards afterfact-observations
add_dep afterfact afterfact-scorecard-publish afterfact-scorecards
add_dep afterfact afterfact-attribution afterfact-event-v2
add_dep afterfact afterfact-attribution hindsight-pov
add_dep afterfact afterfact-attribution hindsight-w5w

add_dep cautionlight cautionlight-readonly-cutover cautionlight-findings

add_dep provenance provenance-afterfact-corpus provenance-5fu
add_dep provenance provenance-afterfact-corpus provenance-a2g
add_dep provenance provenance-afterfact-corpus provenance-f7d
add_dep provenance provenance-afterfact-corpus provenance-srt

add_dep gauntlet gauntlet-undertake-corpus gauntlet-lj5
add_dep gauntlet gauntlet-undertake-corpus gauntlet-289
add_dep gauntlet gauntlet-undertake-corpus gauntlet-be9

add_dep envoy envoy-undertake-corpus envoy-6p5
add_dep envoy envoy-undertake-corpus envoy-ct9
add_dep envoy envoy-undertake-corpus envoy-4yr
