...
- [ ] **`gauntlet-m6`** (P1, **lead-floor**, complexity L) — gauntlet's final v1 milestone, now unblocked with a sound conjunctive verify∧judge signal. Expensive metered workload: run M5's A/B machinery across same-tier candidates, derive lean/std/heavy from cost+latency at equal quality, emit a **proposed** `tiers.md` patch + harness-deck approval block (pending-human handoff; never applies the patch).
